...
"""Progit specific hook to update pull-requests stored in the database
...
def get_username():
    ...
    if progit.APP.config['REQUESTS_FOLDER'] in repo:
        username = repo.split(progit.APP.config['REQUESTS_FOLDER'])[1]
    return username
    ...
    for filename in file_list:
        ...
        if data:
            progit.lib.git.update_request_from_git(
                ...
                json_data=data,
                gitfolder=progit.APP.config['GIT_FOLDER'],
                forkfolder=progit.APP.config['FORK_FOLDER'],
                docfolder=progit.APP.config['DOCS_FOLDER'],
                ticketfolder=progit.APP.config['TICKETS_FOLDER'],
                requestfolder=progit.APP.config['REQUESTS_FOLDER'],
            )
